...
from poserie.common.arguments import parse_args
import torch
import random
import cv2
...
from poserie.common.camera import *
from poserie.common.model import *
from poserie.common.loss import *
from poserie.common.generators import ChunkedGenerator, Evaluate_Generator
from time import time
from poserie.common.utils import deterministic_random
...
def create_pose3D_rie_keypoints_visualization(keypoints: dict,
    ...
    index = 0
    coef = 3
    for pose in tqdm(keypoints_example, desc="Save skelet points"):
        img = np.zeros([int(np.max(keypoints_example) * coef), int(np.max(keypoints_example) * coef), 3],
                       dtype=np.uint8)
        img[:] = 255

        for idx, point in enumerate(pose):
            cv2.circle(img, (int(point[0] * coef), int(point[1] * coef)), 10, (0, 255, 0), -1)
            img = cv2.putText(img, str(idx), (int(point[0] * coef) - 30, int(point[1] * coef) - 20),
                              cv2.FONT_HERSHEY_SIMPLEX,
                              1, (0, 255, 0), 2, cv2.LINE_AA)

        for connection in H36M_POSE_CONNECTIONS:
            cv2.line(img, (int(pose[connection[0]][0] * coef), int(pose[connection[0]][1]) * coef),
                     (int(pose[connection[1]][0] * coef), int(pose[connection[1]][1]) * coef), (255, 0, 0), 4)
...
# Evaluate
def evaluate(model_pos, test_generator, joints_left, joints_right, action=None, return_predictions=False,
...
def run(checkpoint_dir,
    ...
    elif dataset_name.startswith('humaneva'):
        from poserie.common.humaneva_dataset import HumanEvaDataset
    ...
    keypoints = np.load('poserie/data/data_2d_' + dataset_name + '_' + keypoints_param + '.npz', allow_pickle=True)
    ...
    subjects_train = subjects_train_param.split(',')
    if not render_param:
        subjects_test = subjects_test_param.split(',')
    else:
        subjects_test = [viz_subject_param]
    ...
    if pretrain_weights:
        pretrain_filename = os.path.join(checkpoint_dir, pretrain_weights)
    ...
    if args.resume or evaluate_param or args.finetune:
        filename = ""
        if args.resume != "":
    ...
    if not evaluate_param:
        cameras_train, poses_train, poses_train_2d = fetch(subjects_train, action_filter, subset=subset_param)

        lr = lr_param

        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model_pos_train.parameters()), lr=lr, amsgrad=True)

        lr_decay = lr_decay_param
        ...
        while epoch < epochs_param:
            ...
            momentum = initial_momentum * np.exp(-epoch / epochs_param * np.log(initial_momentum / final_momentum))
            model_pos_train.set_bn_momentum(momentum)

            # Save checkpoint if necessary
            if epoch % args.checkpoint_frequency == 0:
                chk_path = os.path.join(checkpoint_dir, 'stage_' + str(stage_param) + '_epoch_{}.bin'.format(epoch))
    ...
    if render_param:
        ...
        input_keypoints = keypoints[viz_subject_param][viz_action_param][viz_camera_param].copy()
        ground_truth = None
        if viz_subject_param in dataset.subjects() and viz_action_param in dataset[viz_subject_param]:
            if 'positions_3d' in dataset[viz_subject_param][viz_action_param]:
                ground_truth = dataset[viz_subject_param][viz_action_param]['positions_3d'][viz_camera_param].copy()
...
if __name__ == "__main__":
    ...
    dataset_name = "h36m"
    pretrain_weights = "gt_pretrained.bin"
    keypoints_param = "gt"
    subjects_train_param = "S1,S5,S6,S7,S8"
    subjects_test_param = "S9,S11"
    subset_param = 1
    stage_param = 3
